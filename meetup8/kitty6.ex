...
defmodule MyGenericServer do
  ...
  def init(module, initial_state) do
    state = module.init(initial_state)
    loop(module, state)
  end
  ...
  def reply({reply_to_pid, ref}, reply) do
    send(reply_to_pid, {ref, reply})
  end
  ...
  def handle_call({:order, name, color, description}, from, cats) do
    case cats do
      ...
      [] ->
        ...
        MyGenericServer.reply(from, new_cat)

        cats

      [first_cat | other_cats]  ->
        MyGenericServer.reply(from, first_cat)
        other_cats
    end
  end
...
defmodule KittyShopTest do
  ...
  def run do
    kitty_shop_pid = MyGenericServer.start_link(KittyShop, [])
